...
void addEdge(int start, int stop, int weigth, int graf[][3], int &dlugosc){
    bool found = false;
    for(int i=0; i<dlugosc; i++){
        if(graf[i][0] == start && graf[i][1] == stop && start != stop){
            graf[i][2] += weigth;
            found = true;
        }
    }
    if(found == false && start != stop){
        graf[dlugosc][0] = start;
        graf[dlugosc][1] = stop;
        graf[dlugosc][2] = weigth;
        dlugosc++;
    }
    ...
    if(start == stop){
        ...
        if(pathExists(start, stop, length, flow, taken, takenV)){
            ...
            for(int i=0; i<length; i++){
                if(taken[i]==1){
                    flow[i][2] -= minimum;
                    addEdge(flow[i][1], flow[i][0], minimum, flow, length);
                }
            }
            // writeGraph(length, flow);
            maxFlow+=minimum;
...
int main(){
    int dlugosc = getLength("graf.txt");
    int graf[dlugosc*2][3];                                                                               
    ifstream file;
    file.open("graf.txt");
    char c;
    file.get(c);
    ...
    if (maxFlow != 0) cout << "Maksymalny przeplyw miedzy tymi wierzcholkami wynosi: " <<maxFlow;
    file.close();
}